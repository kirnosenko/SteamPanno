...
set GODOT_PATH="C:\Program Files\Godot_v4.5.1-stable_mono_win64\Godot_v4.5.1-stable_mono_win64.exe"
...
:: Run godot export
%GODOT_PATH% --verbose --headless --path "%PROJECT_PATH%" --export-release "Linux" "%EXPORT_PATH%\SteamPanno.exe"
if %errorlevel% neq 0 (
...
copy "%PROJECT_PATH%\addons\steam\linux\libsteam_api.so" "%EXPORT_PATH%\libsteam_api.so"
if %errorlevel% neq 0 (
